...
from .logging_config import setup_logging, get_logger, get_error_tracker
...
def check_env() -> bool:
    ...
    if missing_vars:
        console.print(
            f"[red]Fehlende Umgebungsvariablen: {', '.join(missing_vars)}[/red]\n"
...
def cli(ctx, debug):
    ...
    try:
        ...
        console.print(f"[red]Fehler bei Datenbank-Initialisierung: {e}[/red]")
        logger.error(f"Datenbank-Initialisierung fehlgeschlagen: {e}", exc_info=True)
        sys.exit(1)
...
@click.option('--limit', type=int, default=None, help='Maximale Anzahl der zu verarbeitenden Nachrichten')
@click.option('--output', '-o', type=click.Path(file_okay=False), default='downloads',
              help='Ausgabeverzeichnis für die heruntergeladenen Dateien')
@click.option('--parallel', '-p', type=int, default=3, 
              help='Maximale Anzahl paralleler Downloads (Standard: 3)')
@click.pass_context
def download(ctx, group: str, limit: Optional[int], output: str, parallel: int):
    ...
    downloader = AudioDownloader(download_dir=str(output_path), max_concurrent_downloads=parallel)
        ...
        with Progress(
            ...
            async def run_download():
                ...
                try:
                    ...
                    progress.update(task, description=f"Lade Audiodateien aus Gruppe '{group}' herunter...")
                    ...
                    if error_summary['total'] > 0:
                        ...
                        if ctx.obj.get('DEBUG'):
                            ...
                            for error in error_summary['recent']:
                                console.print(f"  - {error['type']} in {error['context']} ({error['time']})")
                    
                except Exception as e:
                    error_tracker.track_error(e, "cli_download", "ERROR")
                    logger.error(f"Download-Fehler: {e}", exc_info=ctx.obj.get('DEBUG', False))
                    console.print(f"[red]Download fehlgeschlagen: {e}[/red]")
                    sys.exit(1)
...
@click.option('--status', type=click.Choice([s.value for s in DownloadStatus]), 
              help='Filtere nach Download-Status')
@click.option('--limit', type=int, default=10, help='Maximale Anzahl der anzuzeigenden Ergebnisse')
...
@click.option('--fuzzy', '-f', is_flag=True, help='Aktiviere Fuzzy-Suche (unscharfe Suche)')
...
@click.option('--duration-min', type=int, help='Minimale Dauer in Sekunden')
@click.option('--duration-max', type=int, help='Maximale Dauer in Sekunden')
def search(query: Optional[str], group: Optional[str], status: Optional[str], 
          limit: int, show_all: bool, metadata: bool, fuzzy: bool,
          min_size: Optional[str], max_size: Optional[str], 
          audio_format: Optional[str], duration_min: Optional[int], duration_max: Optional[int]):
    ...
    for file in files:
        ...
        row_data.extend([
            ...
            file.downloaded_at.strftime("%d.%m.%Y %H:%M") if file.downloaded_at else "-"
        ])
...
def groups():
    ...
    for group in groups:
        ...
        table.add_row(
            ...
            group.last_checked.strftime("%d.%m.%Y %H:%M") if group.last_checked else "-",
            str(file_count)
        )
...
@cli.command()
@click.option('--update', '-u', is_flag=True, help='Aktualisiere Metadaten aus bereits heruntergeladenen Dateien')
...
def metadata(update: bool, verify: bool, file_id: Optional[str]):
    ...
    from .utils import extract_audio_metadata, calculate_file_hash
    from pathlib import Path
    ...
    with Progress(
        ...
        for file in files:
            if not file.local_path or not Path(file.local_path).exists():
                error_count += 1
                progress.update(task, advance=1, description=f"Datei nicht gefunden: {file.file_name}")
                continue
            ...
            try:
                ...
                if verify:
                    if file.checksum_md5:
                        ...
                        if current_checksum == file.checksum_md5:
                            ...
                            console.print(f"[red]Checksum-Fehler: {file.file_name}[/red]")
                            error_count += 1
                    else:
                ...
                progress.update(task, advance=1, description=f"Verarbeitet: {file.file_name}")
...
def stats():
    ...
    completed = AudioFile.select().where(AudioFile.status == DownloadStatus.COMPLETED.value).count()
    failed = AudioFile.select().where(AudioFile.status == DownloadStatus.FAILED.value).count()
...
@click.option('--cleanup', '-c', is_flag=True, help='Bereinige Temp-Dateien und führe Garbage Collection durch')
@click.option('--output', '-o', type=click.Path(file_okay=False), default='downloads',
...
def performance(watch: bool, cleanup: bool, output: str):
    ...
    if cleanup:
        ...
        # Memory Cleanup
        freed_objects = perf_monitor.memory_manager.force_garbage_collection()
        console.print(f"[green]✓ Garbage Collection: {freed_objects} Objekte bereinigt[/green]")
        
        # Temp-Files Cleanup
        cleaned_files = perf_monitor.disk_manager.cleanup_temp_files()
        console.print(f"[green]✓ Temp-Dateien bereinigt: {cleaned_files} Dateien[/green]")
        
        return
    
    if watch:
        console.print("[blue]👁️  Performance-Überwachung gestartet (Strg+C zum Beenden)[/blue]")
        ...
        try:
            while True:
                ...
                console.print(f"[dim]Laufzeit: {report['uptime_seconds']:.0f}s | {time.strftime('%H:%M:%S')}[/dim]\n")
                ...
                downloads_table.add_row("Erfolgreich", str(report['downloads']['completed']))
                downloads_table.add_row("Fehlgeschlagen", str(report['downloads']['failed']))
                downloads_table.add_row("Erfolgsrate", f"{report['downloads']['success_rate']:.1f}%")
                downloads_table.add_row("Downloads/Min", f"{report['performance']['downloads_per_minute']:.1f}")
                downloads_table.add_row("Ø Geschwindigkeit", f"{report['performance']['average_speed_mbps']:.1f} MB/s")
                downloads_table.add_row("Gesamt heruntergeladen", f"{report['performance']['total_gb_downloaded']:.2f} GB")
                ...
                memory_status = "🟢" if memory_mb < 512 else "🟡" if memory_mb < 1024 else "🔴"
                system_table.add_row("Arbeitsspeicher", f"{memory_mb:.0f} MB", memory_status)
                ...
                system_table.add_row("Freier Speicher", f"{disk_free:.1f} GB", disk_status)
                ...
                rate_table.add_row("Aktuelle Rate", f"{report['rate_limiting']['current_rate']:.2f} req/s")
                rate_table.add_row("Verfügbare Tokens", f"{report['rate_limiting']['tokens_available']:.1f}")
        ...
        console.print(f"  Erfolgreich: [green]{report['downloads']['completed']}[/green]")
        ...
        console.print(f"  Gesamt heruntergeladen: {report['performance']['total_gb_downloaded']:.2f} GB")
        console.print(f"  Ø Geschwindigkeit: {report['performance']['average_speed_mbps']:.1f} MB/s\n")
        ...
        console.print(f"  Festplatte verwendet: {report['resources']['disk_used_gb']:.1f} GB")
        console.print(f"  Festplatte frei: {report['resources']['disk_free_gb']:.1f} GB\n")
        
        # Rate-Limiting
        console.print("[bold]⏱️  Rate-Limiting[/bold]")
        console.print(f"  Aktuelle Rate: {report['rate_limiting']['current_rate']:.2f} Anfragen/Sekunde")
        console.print(f"  Verfügbare Tokens: {report['rate_limiting']['tokens_available']:.1f}")
        ...
        if report['resources']['memory_mb'] > 1024:
            console.print("  [yellow]⚠️  Hoher Speicherverbrauch - erwägen Sie --cleanup[/yellow]")
        ...
        if report['rate_limiting']['current_rate'] < 0.5:
            console.print("  [blue]ℹ️  Rate-Limiting aktiv - Downloads verlangsamt[/blue]")
        
        if (report['resources']['memory_mb'] <= 512 and 
            report['resources']['disk_free_gb'] > 5 and
            report['rate_limiting']['current_rate'] >= 1.0):
