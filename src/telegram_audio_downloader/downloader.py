...
class AudioDownloader:
    ...
    def __init__(self, download_dir: str = "downloads", max_concurrent_downloads: int = 3):
        """
    ...
    async def initialize_client(self) -> None:
        ...
        if not all([api_id, api_hash]):
            raise ValueError("API_ID und API_HASH müssen in der .env-Datei gesetzt sein")
    ...
    def _load_downloaded_files(self) -> None:
        ...
        self._downloaded_files = {
            audio.file_id for audio in AudioFile.select(AudioFile.file_id).where(
                AudioFile.status == DownloadStatus.COMPLETED.value
            )
        }
        logger.info(f"{len(self._downloaded_files)} bereits heruntergeladene Dateien geladen")
    ...
    def _is_audio_file(self, document: Document) -> bool:
        ...
        if document.mime_type and any(audio_type in document.mime_type for audio_type in ['audio', 'mpeg']):
            return True
    ...
    def _extract_audio_info(self, document: Document) -> Dict[str, Any]:
        ...
        audio_attrs = next(
            (attr for attr in document.attributes if isinstance(attr, DocumentAttributeAudio)),
            None
        )
    ...
    async def download_audio_files(self, group_name: str, limit: Optional[int] = None) -> None:
        """
        ...
        try:
            ...
            logger.info(f"Gruppe gefunden: {group_entity.title} (ID: {group_entity.id})")
            ...
            async for message in self.client.iter_messages(group_entity, limit=limit):
                ...
                if file_id in self._downloaded_files:
                    logger.debug(f"Überspringe bereits heruntergeladene Datei: {file_id}")
                    continue
            ...
            logger.error(f"Fehler beim Herunterladen der Audiodateien: {e}", exc_info=True)
            raise
    
    async def _download_audio_concurrent(self, message: Message, document: Document, group: TelegramGroup) -> bool:
        """
    ...
    async def _download_audio(self, message: Message, document: Document, group: TelegramGroup) -> None:
        ...
        try:
            ...
            if not created and audio_file.is_downloaded:
                logger.debug(f"Datei bereits heruntergeladen: {audio_info['file_name']}")
            ...
            if audio_file.can_resume_download():
                ...
                if partial_size == audio_file.downloaded_bytes:
                    ...
                    logger.info(f"Setze Download fort ab Byte {start_byte}: {audio_info['file_name']}")
                else:
                    logger.warning(f"Partielle Datei inkonsistent, starte neu: {audio_info['file_name']}")
            ...
            logger.warning(f"FloodWaitError: Warte {wait_time} Sekunden für {audio_info['file_name']}...")
            ...
            if error_tracker.should_retry(e, f"network_{file_id}"):
                retry_delay = min(60, 2 ** audio_file.download_attempts)  # Exponential backoff, max 60s
                logger.warning(f"Netzwerk-Fehler bei {audio_info['file_name']}, Retry in {retry_delay}s: {e}")
            ...
            logger.error(f"Unerwarteter Fehler beim Herunterladen der Datei {file_id}: {e}", exc_info=True)
    ...
    async def _download_with_resume(self, message: Message, file_path: Path, 
                                    start_byte: int, audio_file: AudioFile) -> int:
        """
        ...
        while retry_count < max_retries:
            try:
                ...
                if file_path.exists() and file_path.stat().st_size > audio_file.file_size:
                    file_path.unlink()
                    start_byte = 0
                ...
                if retry_count < max_retries:
                    ...
                    logger.error(f"Download fehlgeschlagen nach {max_retries} Versuchen: {e}")
                    raise
