...
class TelegramAudioLogger:
    ...
    def log_download_progress(self, file_name: str, progress: float, speed: Optional[float] = None):
    ...
    def log_api_error(self, operation: str, error: Exception, retry_count: int = 0):
    ...
    def log_system_info(self):
        ...
        self.logger.info("SYSTEM_INFO: " + " | ".join([f"{k}: {v}" for k, v in info.items()]))
...
class ErrorTracker:
    ...
    def __init__(self):
        self.errors = []
        self.error_counts = {}
        self.logger = TelegramAudioLogger().get_logger()
    
    def track_error(self, error: Exception, context: str, severity: str = "ERROR"):
        ...
        log_method(f"TRACKED_ERROR [{context}]: {error_info['type']} - {error_info['message']}")
    ...
    def get_error_summary(self) -> dict:
        ...
        return {
            ...
            "by_type": by_type,
            "most_common": sorted(self.error_counts.items(), key=lambda x: x[1], reverse=True)[:3],
            "recent": [
                {
                    "type": e["type"],
                    "context": e["context"],
